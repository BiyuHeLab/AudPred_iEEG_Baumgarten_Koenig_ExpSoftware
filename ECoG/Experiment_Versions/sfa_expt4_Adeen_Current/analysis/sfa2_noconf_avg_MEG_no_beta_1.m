...
path_fig = '/data/gogodisk1/amy/data/sfa_expt2_noconf/analysis/figures/avg_no_beta=1/';
mkdir(path_fig)
...
% filter_beta = data.stim.betaID ~= 2 & data.stim.betaID ~= 4; % omit beta=.5 and 1.5 stim
% filter_beta = data.stim.betaID ~= 3 & data.stim.betaID ~= 4; % omit beta=1 and 1.5 stim
% filter_beta = data.stim.betaID ~= 2 & data.stim.betaID ~= 3; % omit beta=.5 and 1 stim

filter_beta = data.stim.betaID ~= 3; % omit beta=1 stim
...
filter = filter_resp & filter_rt & filter_manual & filter_beta;
...
for i_b = 2:length(betas)
    for i_r = 1:length(betas)-1
...
errorbar(betas, [0 dcum_avg], [0 dcum_sem], 'bo');
pp = polyfit(betas(2:end), dcum_avg, 1);
xx = [.5 2];
