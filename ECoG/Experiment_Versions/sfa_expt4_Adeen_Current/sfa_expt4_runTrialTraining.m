function [data exitNow] = sfa_expt4_runTrialTraining(window, param, stim, trialNum, timingInd, data)
...
%Trigger: long square wave (2*4410 data points) at the beginning of sequence (i.e., tone 1), 
%short square wave (1*4410 data points) at end of sequence (i.e., final tone)
...
data.timing.dur_loop(timingInd) = GetSecs - t0;%current time - time buffering started
...
%% sort out response for task 1
% sort out correctness (not definitive, only close or not close)
%if p34 & p*34 are close and response indicates high likelihood
if data.stim.match(trialNum) == 1 && data.resp_prob(trialNum)>3
   data.likelihood_estimate(trialNum) = 2;
%if p34 & p*34 are not close and response indicates low likelihood
elseif data.stim.match(trialNum) == 0 && data.resp_prob(trialNum)<3 && data.resp_prob(trialNum)> 0 
   data.likelihood_estimate(trialNum) = 2;  
elseif data.resp_prob(trialNum) == 3 %unsure response
   data.likelihood_estimate(trialNum) = 1; 
elseif data.resp_prob(trialNum)<0 %no (valid) response given
   data.likelihood_estimate(trialNum) = -1; 
else %otherwise, just far off likelihood estimate
    data.likelihood_estimate(trialNum) = 0;
end

if exitNow, return; end

%% show feedback
%Read out trend of current sequence based on p*34
if stim.logf_pred(trialNum) < 6.08
    sequence_trend = 'falling';
elseif stim.logf_pred(trialNum) > 6.09
    sequence_trend = 'rising';
else
    sequence_trend = 'stable';
end

%Read out current p34 (categorized in low, middle, high)
if stim.finalID(trialNum) == 2
    sequence_p34 = 'very high';
elseif stim.finalID(trialNum) == 1
    sequence_p34 = 'relatively high';
elseif stim.finalID(trialNum) == -1
    sequence_p34 = 'relatively low';
elseif stim.finalID(trialNum) == -2
    sequence_p34 = 'very low';
end

%Check given likelihood rating (categorized in low vs. unsure vs. high)
if data.resp_prob(trialNum) < 3
str_likelihoodrating = 'unlikely';
elseif data.resp_prob(trialNum) > 3
str_likelihoodrating = 'likely';
else
str_likelihoodrating = 'unsure';
end

switch data.likelihood_estimate(trialNum)
    case 2,  FB_text = ['Accurate estimate\n\n' ...
            'The sequence had a ' sequence_trend ' trend and a '  sequence_p34 ' final tone pitch.\n\n' ...
            'You rated the likelihood of the final tone pitch as ' str_likelihoodrating];
    case 1,  FB_text = ['Unsure estimate\n\n' ...
            'The sequence had a ' sequence_trend ' trend and a '  sequence_p34 ' final tone pitch.\n\n'];
    case 0,  FB_text = ['Inaccurate estimate\n\n' ...
            'The sequence had a ' sequence_trend ' trend and a '  sequence_p34 ' final tone pitch.\n\n' ...
            'You rated the likelihood of the final tone pitch as ' str_likelihoodrating];
