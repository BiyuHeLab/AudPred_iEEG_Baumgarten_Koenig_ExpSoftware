function param = sfa_expt4_getParamsTraining(window, freq, isBB, isMEG)
...
param.nTonesInSeries   = length(series_Hz{3}(1,:)) + 1; %34 tones in total
...
param.fixationBuffer_inSecs = .5; %.3; % time between offset of last trial's fixation buffer (FB) and onset of this trial's fixation dot
...
param.textSizeInstructions = 25; %50
param.textSize  = param.textSizeTrial;
param.sx        = 200;
param.vSpacing  = 1.4;
param.vSpacingInstructions  = 1.6;
...
% response time (in s) allowed for each response before triggering next trial automatically
param.respDur_inSecs = 60;
